...
{
    sealed public class GamepadReaderAnalogToDPad : IControllerReader
    {
        ...
        public GamepadReaderAnalogToDPad(int id = 0)
        {
        ...
        void tick(object sender, EventArgs e)
        {
            ...
            float x = (float)state.X / RANGE;
            float y = (float)state.Y / RANGE;

            outState.SetAnalog("x", x);
            outState.SetButton(x < 0 ? "left" : "right", x != 0);
            outState.SetAnalog("y", y);
            outState.SetButton(y < 0 ? "up" : "down", y != 0);
